...
import type { Route } from "./+types/top-featured-products-edit.page";
...
export async function action({ request, context }: Route.ActionArgs) {
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.update(schema.products).set({ homeFeaturedOrder: -1 });
    ...
    logger.info("Home featured products updated", { adminId, count: args.length });

    return data(
      {
        success: true,
        error: null,
        message: "Projekty wyróżnione zostały zaktualizowane",
        products: [],
      },
      { status: 200 }
    );
  } catch (error) {
    logger.error("Failed to update home featured products", { error, adminId });
    throw data(
      {
        success: false,
        error: error,
...
export default function TopFeaturedProductsEditPage({
  loaderData,
  ...
  const [productIds, setProductIds] = React.useState<string[]>(
    products
      .filter(
        (p) => p.homeFeaturedOrder !== undefined && p.homeFeaturedOrder >= 0
      )
      .map((p) => p.id)
  );

  const isUpdating = navigation.formAction === "/admin/wyr-projekty";
  ...
  const onSave = (productIds: string[]) => {
    ...
    const data = productIds.map((id) => ({
      id,
      homeFeaturedOrder: productIds.indexOf(id),
    }));
  ...
  return (
    <AdminPageContainer>
      ...
      <AdminPageContent>
        <FieldSet>
          <FieldLegend>Wyróżnione projekty</FieldLegend>
          <FieldDescription>
            Przeciągnij i upuść projekty, aby zmienić kolejność. Użyj przycisków
            strzałek lub przeciągania, aby uporządkować wyróżnione projekty.
          </FieldDescription>
